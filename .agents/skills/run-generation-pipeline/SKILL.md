...
横髪用比較を併用する場合は`--side-comparison`とDEVELOPMENT.mdの比較仕様版17手順を使う。生成済み画像の原寸・原画/解析の同一性と公開前SHAを検査する。可視の耳輪郭を変える`--redraw-ear-contour`は利用者の承認を得た場合だけ使う。生成画像と原画の耳をそれぞれ既存DINO/SAMで抽出し、色差だけで耳全体の範囲を代用しない。原画ファイル・目口・許可領域外の画素を保持する検査を通す。輪郭修正版では中立も変わるため、原画全画素固定と呼ばない。通常の非描き直し比較は中立RGBA一致を必須とする。
