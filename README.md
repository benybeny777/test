...
動作での比較は従来の`http://127.0.0.1:8791/ui/check.html`で「むぎ・原画固定＋補完候補」を選びます。女性A・むぎ・実写テストは従来どおり残しています。候補は原画の不透明な髪に隠れる局所下地だけを追加し、中立の合成画素を保持します。「顔左右」「顔上下」「待機動作テスト」で小さな髪の相対移動を試し、「補完下地を表示」を外すと同じ動きの欠損を比較できます。大きい変位の髪際、口・閉眼の造形は未合格で、全身独立関節や完成Live2Dモデルではありません。候補素材はローカル生成が必要で、Gitへ同梱していません。
