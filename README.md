...
> **現在は2D/2.5D経路への移行中です。** `isolate → decompose → rig2d` で、原画・部位・目口差分を含む14レイヤーと schema_version 2 の `lvs-anime25d-v1` リグを生成します。目口の実測座標、口の2軸変形、原画を貼った連続メッシュによる小角度の動きを実装中です。**ひより相当の品質や別画像での汎用性は未達・検証中**です。詳細は [docs/TASKS.md](docs/TASKS.md) T11/T12を参照してください。

## できること

開発用の意味解析モデル比較は[比較手順](docs/DEVELOPMENT.md#意味解析候補の比較製品未採用)を参照してください。Florence-2/Grounding DINOの実機比較は通常の生成経路とは別で、まだ製品へ採用していません。
