...
> **現在は設計段階です。** 実装はまだありません。作業に入る前に [docs/TASKS.md](docs/TASKS.md) を読んでください。
>
> 製品名 `LocalVTuberStudio` と識別子 `com.localvtuberstudio.desktop` は仮称です。
...
| GPU | **CUDA対応 NVIDIA GPU、VRAM 8GB 以上を推奨。** GPU 非搭載環境では画像→3D生成が使えません |
| RAM | 16GB 以上 |
| ディスク | モデル一式で 15GB 程度（実測後に確定） |
| マイク | リップシンクと音声会話に必要 |
