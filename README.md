...
動作での比較は従来の`http://127.0.0.1:8791/ui/check.html`で「むぎ・耳輪郭修正候補」を選びます。女性A・むぎ・実写テストは従来どおり残しています。「顔を拡大」で接合部を見て、「顔左右」「顔上下」「待機動作テスト」で小さな髪の相対移動を確認できます。耳は元より大きく露出する形に変わるため、利用者の見た目の判断が必要です。髪際、口・閉眼の造形は未合格で、全身独立関節や完成Live2Dモデルではありません。候補素材はローカル生成が必要で、Gitへ同梱していません。
