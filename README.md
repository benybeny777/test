...
> **現在は段階実装中です。** デスクトップ外枠、女性3体での画像→3D・自動リギング・VRM出力・11枚へ削減したローカル表情生成・フェイスパッチ逆投影、滑らかな表情遷移、自動まばたき、リップシンク、透過OBS出力まで実装済みです。外部で用意した表情画像も検査して取り込めます。未完了範囲と着手順は [docs/TASKS.md](docs/TASKS.md) を参照してください。
