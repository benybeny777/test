...
実写の対象はPicoAgentの「実写・物理演算テスト」です。[PicoAgent実写・原画から再生成](http://127.0.0.1:8791/ui/check.html?character=c_386f432bc3dd)で、同じ生成工程を通した候補を確認できます。取り違えた別の女性素材とその閉眼候補は一覧から外し、ファイルは保留しています。最終品質は未承認です。
...
動作での比較は従来の`http://127.0.0.1:8791/ui/check.html`で「むぎ・耳輪郭修正候補」を選びます。女性A・むぎは従来どおり残しています。実写はPicoAgentの依頼対象へ差し替える準備中です。「顔を拡大」で接合部を見て、「顔左右」「顔上下」「待機動作テスト」で小さな髪の相対移動を確認できます。耳は元より大きく露出する形に変わるため、利用者の見た目の判断が必要です。髪際、口・閉眼の造形は未合格で、全身独立関節や完成Live2Dモデルではありません。候補素材はローカル生成が必要で、Gitへ同梱していません。
