...
export function createAvatarRenderer(canvas) {
  ...
  async function applyState(next) {
    ...
    if(next.rigUrl!==state.rigUrl || !rig) {
      ...
      if(incoming.eye_rig_version!==3)throw new Error('白目の下地がない旧リグです。分解から再生成してください');
      for(const side of ['left','right'])eyeAperture(incoming.layers?.[side+'_eye_base'],1);
      const names=['neutral','mouth_open','mouth_closed',...['left','right'].flatMap(side=>[side+'_eye_iris',side+'_eye_backplate',side+'_eye_base',side+'_eyelid_upper'])];
