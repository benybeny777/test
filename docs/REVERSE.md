...
## リップシンク（`RM_MicrophoneLipSyncDriver` 実測）

完全ローカル。機械学習を使わない**FFT＋フォルマント最近傍**方式。
...
1. マイクリングバッファから直近512サンプルを取得
2. RMS → dB。ゲート未満なら無音扱い
3. **Hann窓**を掛ける
4. **素朴なDFT**（O(N²)、512点で実装。FFTではない）で振幅スペクトルを得る
5. 前後3ビンの移動平均で**スペクトル包絡**を作る
6. F1 を 250〜1000Hz の最大ピークとして探す
7. F2 を「F1+150Hz」以降、700〜3200Hz の最大ピークとして探す
8. 各母音との距離を **対数周波数空間のユークリッド距離** `log(F1/f1)^2 + log(F2/f2)^2` で計算。A のみ `aBias` を乗算
9. 最小距離の母音を採用 → 4フレーム多数決 → 変化したときだけテクスチャ差し替え

フォールバックが丁寧で、目的の母音テクスチャが無ければ `__a` → `close` → 表情本体、の順に落ちる。

VAD も同居しており、発話区間（終端無音0.9秒 / 最短0.5秒 / 最長6秒）を切り出して `OnUtteranceRecorded` で配る。これが whisper.cpp へ渡り AITuber の会話になる。

## OBS 連携（`RM_ObsStreamService` / `RM_ObsMjpegServer` 実測）

**ローカルMJPEG HTTPサーバ**。`127.0.0.1` の **58090〜58099** を順に試して空きポートへバインドし、`http://127.0.0.1:<port>/` を OBS のブラウザソースに貼らせる。プラグイン不要はこの構造による。

- 有効状態は `PlayerPrefs` の `RM_ObsStream_Enabled` に保存し、次回起動で自動復帰する
- 全ポート使用中なら明示エラー（「ポート 58090〜58099 が全て使用中のため開始できませんでした。」）
- `RM_ObsFrameComposer` が合成、`RM_ObsJpegWorker` が JPEG エンコード、`RM_BaselineJpegEncoder` を自前実装している

**MJPEG はアルファチャンネルを持てない**。そのため本家は透過配信ができず、`RM_ChromaKeyCheckView` / `RM_ChromaKeySettings` / `RM_ObsChromaKeyGuidePanel` を用意して**グリーンバック＋OBS側クロマキー**で抜かせている。背景プリセットの既定が `green_screen` なのもこのため。JPEG 再圧縮による色滲みがクロマキー品質を落とす構造的な弱点であり、**クローンが上回れる余地がここにある**。
