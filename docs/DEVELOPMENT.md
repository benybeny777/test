...
比較仕様版17のむぎ候補IDは`c_b4b936129f4d`。利用者承認により可動モデルの耳輪郭を描き直す。原画ファイルは保持するが、中立合成も許可した耳周辺26,120画素が変わる。目口と許可領域外の変更はエラーにする。耳の形で進めることは利用者承認済み。髪との接合部の品質は未合格。
...
比較仕様版17は顔と未分類の両方から髪境界637画素を回収する。髪に完全に囲まれた領域7,934画素（髪飾り・ハイライト等）も、目口を含む閉領域と透明画素を除外して髪へ移す。色は原画のままで、髪だけ動いて飾りの境界が取り残される現象を抑える。狭い色差条件と閉領域条件の汎用性は別原画では未検証。
