...
原画1枚からの通常生成と、過去の比較記録です。動的な確認画面のキャラ一覧には通常生成で公開中の最新世代だけを表示し、旧比較候補は混ぜません。左は変更しない原画、右は本体と共通の2.5Dレンダラーです。2026年9月7日、Chrome・1440×1050の確認画面で撮影しています。新旧の生成条件は各項目に明記し、画像をタップすると拡大表示できます。
...
「口パク動作テスト（無音）」と「自動まばたきを開始」は別々に開始します。「固定口形」で閉口・母音を静止確認できます。動作中は各ボタンが停止表示へ変わります。
