...
def graph(mode,width,height,prompt,steps,seed,layers,region_mask=False):
    ...
    if region_mask:
        if mode!='edit':raise ValueError('局所編集はImage-Editだけで使用します')
...
def feature_edit_mask(features,hair,alpha,margin_ratio):
    """測定された部位を中心に編集範囲を作り、髪・背景を保護する。"""
    if not 0<margin_ratio<=.5:raise ValueError('部位の編集余白が範囲外です')
    region=np.zeros(alpha.shape,dtype=float)
    for feature in features:
        ys,xs=np.nonzero(feature)
        if not xs.size:raise ValueError('部位の編集マスクが空です')
        margin=max(2,round((int(xs.max())-int(xs.min())+1)*margin_ratio))
        expanded=ndimage.binary_dilation(feature,iterations=margin)&~hair&(alpha>0)
        if not np.any(expanded&feature):raise ValueError('可視部位が編集範囲にありません')
...
def main():
    ...
    parser.add_argument('--edit-region',choices=['all','eyes','mouth'],default='all')
    ...
    if args.edit_region!='all' and (args.mode!='edit' or args.view!='head'):raise ValueError('局所編集は原寸頭部のImage-Edit専用です')
    ...
    if args.edit_region!='all':
        ...
        with np.load(args.character/'analysis/masks.npz',allow_pickle=False) as masks:
            roles=['left_eye','right_eye'] if args.edit_region=='eyes' else ['mouth']
            mask=feature_edit_mask([masks[role][t:b,l:r] for role in roles],masks['hair'][t:b,l:r],alpha,args.mask_margin_ratio)
        ...
        source.update(edit_region=args.edit_region,mask_margin_ratio=args.mask_margin_ratio,edit_mask_sha256=digest(output/'input/eye-mask.png'))
    ...
    workflow=graph(args.mode,*size,prompt,args.steps,args.seed,args.layers,args.edit_region!='all')
            ...
            while time.monotonic()-generation_started<args.timeout:
                ...
                if queued['prompt_id'] in history:
                    ...
                    if items:
                        ...
                        if args.edit_region!='all' and digest(output/'input/eye-mask.png')!=source['edit_mask_sha256']:raise ValueError('実行中に編集マスクが変更されました')
